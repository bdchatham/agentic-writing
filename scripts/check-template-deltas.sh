#!/usr/bin/env bash
# The spec template is a fork of the one GitHub Spec Kit ships. `specify init`
# rewrites .specify/templates/ from the CLI's bundled assets, so a re-init or a
# CLI upgrade silently reverts it and takes the deltas with it.
#
