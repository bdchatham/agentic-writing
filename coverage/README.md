...
The registry says `coverage: partial`. This directory says *partial of what*. It
is the artifact you hand a teammate who asks "does this catch X?", and it is
what makes the single-source-of-truth claim checkable instead of asserted.
...
answer and is expected to be common.
...
3. A rule is claimed by more than one topic in more than one file.
...
coverage was delivered by rules checking something else. That is now a build
failure rather than a reading comprehension exercise.

## What the test does not catch yet

Reviewed and deliberately deferred, so the gap is written down rather than
assumed away:
...
and no rule text is copied. `total_topics` records a count the standard states
about itself. See `NOTICE.md`.
